...
//    添加歌曲
    ...
    public Object addSong(HttpServletRequest req, @RequestParam("file") MultipartFile mpfile){
        ...
        String singer_id = req.getParameter("singerId").trim();//歌手id
        String name = req.getParameter("name").trim();//歌曲名
        String introduction = req.getParameter("introduction").trim();//简介
        String pic = "/img/songPic/tubiao.jpg";//默认图标
...
//    删除歌曲
    ...
    public Object deleteSong(HttpServletRequest req){
        ...
        Song song = songService.songOfId(songId);
        if (song != null) {
            String filePath = System.getProperty("user.dir") + System.getProperty("file.separator") + song.getUrl().substring(1);
